...
if (!mxG.P) {
        ...
        while (K < this.len) {
            switch (a = this.s.charAt(K)) {
                ...
                case ')':
                    return K;
                default:
                    if ((a >= 'A') && (a <= 'Z')) K = this.parseProperty(K); else K++;
            }
        }
        return K;
    };
    ...
    mxG.P.prototype.parseSgf = function (s) {
        ...
        while (K < this.len) if (this.s.charAt(K) == '(') {
            K++;
        ...
        if ((a == "_TB_") || (a == "_TW_"))
            ...
            switch (a) {
                ...
                case "_CR_":
                    ...
                    break;
                default:
                    s += this.makeText(a, i, j, {c: c, cls: cls, sw: this.sw4text});
            }
        }
        return s;
    };
...
if (!mxG.G.prototype.createMenu) {
    ...
    mxG.G.prototype.toggleMenu = function (m, s) {
        ...
        if (s) {
            ...
            this.toggleMenuTimeout = setTimeout(function () {
                mxG.D[z].toggleMenu(m, 0);
            }, t);
        } else {
            this.currentMenu = "";
    ...
    mxG.G.prototype.doFile = function () {
        this.doMenu("File");
    };
    mxG.G.prototype.doEdit = function () {
        this.doMenu("Edit");
    };
    mxG.G.prototype.doView = function () {
        this.setViewCoche();
        this.doMenu("View");
    };
    ...
    mxG.G.prototype.doOpenOK = function () {
        ...
        if (e) {
            if ((r.f.name ? r.f.name : r.f.fileName).match(/\.sgf$/)) e.innerHTML = "";
                ...
                e.innerHTML = this.local("This is not a sgf file!");
                return;
            }
        }
...
if (!mxG.G.prototype.createView) {
    ...
    mxG.G.prototype.doZoomPlus = function () {
        this.doZoom("+");
    };
    mxG.G.prototype.doNoZoom = function () {
        this.doZoom("=");
    };
    mxG.G.prototype.doZoomMinus = function () {
        this.doZoom("-");
    };
...
// else return the last move played at (x,y) if any
    ...
    mxG.G.prototype.initGoban = function () {
        ...
        if (this.canGobanFocus) {
            ...
            this.ig.addEventListener("keydown", function (ev) {
                mxG.D[k].doKeydownGoban(ev);
            }, false);
            this.ig.addEventListener("focus", function (ev) {
                mxG.D[k].doFocusGoban(ev);
            }, false);
            this.ig.addEventListener("blur", function (ev) {
                mxG.D[k].doBlur4FocusGoban(ev);
            }, false);
            this.ig.addEventListener("mousedown", function (ev) {
                mxG.D[k].doMouseDown4FocusGoban(ev);
            }, false);
            this.ig.addEventListener("contextmenu", function (ev) {
...
if (!mxG.G.prototype.createNavigation) {
    ...
    mxG.G.prototype.initNavigation = function () {
        ...
        this.ig.addEventListener("wheel", function (ev) {
...
if (!mxG.G.prototype.createVariation) {
    ...
    mxG.G.prototype.initVariation = function () {
        ...
        this.ig.addEventListener("click", function (ev) {
...
if (!mxG.G.prototype.createHelp) {
    ...
    mxG.G.prototype.doHelp = function () {
        if (this.gBox == "ShowHelp") {
            this.hideGBox("ShowHelp");
            return;
        }
...
if (!mxG.G.prototype.createImage) {
    ...
    mxG.G.prototype.doSvg = function () {
        if (this.gBox == "ShowSvg") {
            this.hideGBox("ShowSvg");
            return;
        }
...
if (!mxG.G.prototype.createScore) {
        ...
        for (i = 1; i <= this.DX; i++)
            for (j = 1; j <= this.DY; j++) {
                switch (this.scoreBan.computed[i][j]) {
                    case "B":
                        cb++;
                        if (this.scoreBan.initial[i][j] == "W") pb++;
                        break;
                    case "W":
                        cw++;
                        if (this.scoreBan.initial[i][j] == "B") pw++;
                        break;
                    default:
                ...
                if (!v || v[this.xy(i, j)]) {
                    if (this.scoreBan.modified[i][j]) {
                        switch (this.scoreBan.modified[i][j]) {
                            case "B":
                                this.scoreBan.computed[i][j] = "B";
                                break;
                            case "W":
                                this.scoreBan.computed[i][j] = "W";
                                break;
                            default:
        ...
        switch (this.scoreBan.computed[a][b]) {
            case po:
                switch (this.scoreBan.initial[a][b]) {
                    case opo:
                        this.scoreBan.modified[a][b] = "E";
                        break;
                    default:
                        this.scoreBan.modified[a][b] = opo;
                }
                ;
                break;
            case opo:
    ...
    mxG.G.prototype.checkScore_propagate = function (x, y) {
        ...
        if (((nat == "B") || (nat == "W")) && this.canSwapWhenScore(nat, x, y)) {
            ...
            if (!this.scoreBan.modified[x][y] || (this.scoreBan.modified[x][y] == "E")) {
                ...
                if (z == 1) {
                    this.visitedWhenSwap = [];
                    this.swapWhenScore("W", x, y, d);
                    return;
                }
                if (z == 2) {
                    this.visitedWhenSwap = [];
                    this.swapWhenScore("B", x, y, d);
                    return;
                }
            } else {
    ...
    mxG.G.prototype.initScore = function () {
        ...
        this.ig.getMClick = mxG.getMClick;
        this.ig.addEventListener("click", function (ev) {
            mxG.D[k].doClickScore(ev);
        }, false);
        if (this.canGobanFocus)
            this.ig.addEventListener("keydown",
                function (ev) {
                    mxG.D[k].doKeydownGobanForScore(ev);
                }, false);
        if (this.scoreBtnOn)
...
if (!mxG.G.prototype.createSgf) {
    ...
    mxG.G.prototype.buildAllSgf = function (aN, only, c) {
        ...
        if (aN.Kid && aN.Kid.length) {
            if (only & 4) {
                if (aN != this.cN) aText += this.buildAllSgf(this.kidOnFocus(aN), only, c);
        ...
        if (only & 4) {
            ...
            if ((aN.Dad && (aN.Dad == this.rN)) || (aN.Dad && (aN.Dad.Kid.length > 1))) aText += ")";
        }
        return aText;
    };
...
if (!mxG.G.prototype.createEdit) {
    ...
    mxG.G.prototype.doEditTool = function (newTool) {
        if (this.gBox) {
            if (newTool == this.gBox) this.hideGBox(newTool);
            return;
        }
        if (newTool == "ShowInfo") {
            this.doInfo();
        ...
        if ((newTool == "Play") && (this.editTool == "Play")) {
            if (this.editNextNat == "B") {
                this.editNextNat = "W";
                this.drawSvgTool("Play");
            } else if (this.editNextNat == "W") {
                this.editNextNat = "B";
                this.drawSvgTool("Play");
            }
            return;
        ...
        if ((newTool == "Setup") && (this.editTool == "Setup")) {
            if (this.editAX == "AB") {
                this.editAX = "AW";
                this.drawSvgTool("Setup");
            } else if (this.editAX == "AW") {
                this.editAX = "AB";
                this.drawSvgTool("Setup");
            }
            return;
    ...
    mxG.G.prototype.checkEdit = function (x, y) {
        switch (this.editTool) {
            case "Play":
                this.checkEditPlay(x, y);
                break;
            case "Setup":
            case "SetupBlack":
            case "SetupWhite":
                this.checkEditSetup(x, y, this.editTool);
                break;
            case "Mark":
                this.checkEditMarkOrLabel(x, y, "MA");
                break;
            case "Triangle":
                this.checkEditMarkOrLabel(x, y, "TR");
                break;
            case "Circle":
                this.checkEditMarkOrLabel(x, y, "CR");
                break;
            case "Square":
                this.checkEditMarkOrLabel(x, y, "SQ");
                break;
            case "Label":
                this.checkEditMarkOrLabel(x, y, "LB");
                break;
            case "Select":
                if (mxG.isAndroid) this.doMouseDownEditSelect(x, y);
                break;
        }
    };
    ...
    mxG.G.prototype.drawSvgTool = function (tool) {
        ...
        switch (tool) {
            ...
            case "Triangle":
                ...
                break;
            case "Numbering":
                e.innerHTML = this.scr.makeAloneStone("W", 5, o);
                break;
            case "ShowInfo":
                e.innerHTML = this.scr.makeAloneToolText(this.local("H"));
                break;
            case "AsInBook":
                e.innerHTML = this.scr.makeAloneToolText(this.local("K"));
                break;
            case "Indices":
                e.innerHTML = this.scr.makeAloneToolText(this.local("I"));
                break;
            case "Variation":
                e.innerHTML = this.scr.makeAloneToolText(this.local("V"));
                break;
            case "Style":
                e.innerHTML = this.scr.makeAloneToolText(this.local("S"));
                break;
            case "GB":
                e.innerHTML = this.scr.makeAloneToolText(this.local("●+"));
                break;
            case "GW":
                e.innerHTML = this.scr.makeAloneToolText(this.local("○+"));
                break;
            ...
            case "IT":
                ...
                break;
            case "PL":
                e.innerHTML = this.scr.makeAloneToolText(this.local("T"));
                break;
            case "R":
                e.innerHTML = this.scr.makeAloneToolText(this.local("↺"));
                break;
    ...
    mxG.G.prototype.getToolLabel = function (tool) {
        switch (tool) {
            case "Select":
                return "Selection";
            case "View":
                return "Full/partial view";
            case "Play":
                return "Place a move";
            case "Setup":
                return "Add/remove a stone";
            case "SetupBlack":
                return "Add/remove a black stone";
            case "SetupWhite":
                return "Add/remove a white stone";
            case "Cut":
                return "Cut branch";
            case "Copy":
                return "Copy branch";
    ...
    mxG.G.prototype.initEdit = function () {
        ...
        this.ig.getMClick = mxG.getMClick;
        this.ig.addEventListener("click", function (ev) {
            mxG.D[k].doClickEdit(ev);
        }, false);
        this.ig.addEventListener("mousemove", function (ev) {
            mxG.D[k].doMouseMoveEdit(ev);
        }, false);
        this.ig.addEventListener("mouseup", function (ev) {
            mxG.D[k].doMouseUpEdit(ev);
        }, false);
        this.ig.addEventListener("mousedown", function (ev) {
            mxG.D[k].doMouseDownEdit(ev);
        }, false);
        this.ig.addEventListener("mouseout", function (ev) {
            mxG.D[k].doMouseOutEdit(ev);
        }, false);
        if (this.canGobanFocus)
            this.ig.addEventListener("keydown",
                function (ev) {
                    mxG.D[k].doKeydownGobanForEdit(ev);
                }, false);
        if (this.hasEditTool("Comment")) {
            this.getE("CommentToolDiv").addEventListener("click", function () {
                mxG.D[k].doEditCommentTool();
            }, false);
            this.getE("CommentToolText").addEventListener("change", function () {
                mxG.D[k].doEditCommentTool();
            }, false);
...
if (!mxG.G.prototype.createInfo) {
    ...
    mxG.G.prototype.putInfoInBox = function () {
        var p, pm,
            IX = ["EV", "RO", "DT", "PC", "PB", "BR", "PW", "WR", "HA", "KM", "RE", "GC", "RU", "TM", "OT", "AN", "CP", "SO", "US", "GN", "BT", "WT", "ON"];
        pm = IX.length;
        for (p = 0; p < pm; p++)
            if (this.getE(IX[p])) {
                if (IX[p] == "RE") {
...
if (!mxG.G.prototype.createTree) {
    ...
    mxG.G.prototype.initTree = function () {
        ...
        this.td.addEventListener("scroll", function (ev) {
